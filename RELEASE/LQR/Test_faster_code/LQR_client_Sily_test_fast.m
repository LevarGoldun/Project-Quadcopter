...
% TEST 25.07 URYCHLENI SIMULINK
% GitHub copilot doporucil misto set_param udelat from workspace
...
elseif contains(v, 'R2024b')
    simulink_file_name = 'LQR_simulink_Sily_test_fast'; %verze 2024b (autor)
else
...
% -> init_int_dref прямо в блок, хотя это все еще инициализация
%--------------------------------------------------------------------------
...
% set_param([smfn,'/time'],'Value', '0') % чтем из workspace
time = 0;
...
try
    while true && time<simtime
        ...
        Rotor_AngVel_square = Rotor_AngVel_square(:,end);
