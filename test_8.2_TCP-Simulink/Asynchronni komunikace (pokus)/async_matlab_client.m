...
% Nastavujeme 'Stop Time' v Simulink a pocetecni cas
...
% Используем таймер для периодического выполнения функции exchangeData
timerObj = timer('ExecutionMode', 'fixedRate', 'Period', timestep, 'TimerFcn', ...
    @(t,~)exchangeData(t, tcpObj, simulink_file_name));

% Обновляем out перед каждым запуском таймера
timerObj.UserData = out;
start(timerObj);

function exchangeData(timerObj, tcpObj, simulink_file_name)
    % Получает данные от Python, обрабатывает их и отправляет управляющие сигналы обратно.
    % try
        out = timerObj.UserData; % Получаем актуальное значение

        % БЛЯТЬ ЭТО ЕБАНЫЙ ПИЗДЕЦ КАКОЙ-ТО НАХУЙ. НИ КОПИЛОТ, НИ ЧАТ
        % НЕ МОГУТ БЛЯТЬ РЕШИТЬ КАК ПО МНЕ ЭЛЕМЕНТАРУНЮ ПРОБЛЕМУ
        % ОБНОВИТЬ out ЧТОБЫ ПОСЫЛАТЬ АКТУАЛЬНЫЕ ДАННЫЕ В ПАЙТОН
        % НУ НЕТ НАХУЙ, ОНО БЕРЕТ ТОЛЬКО ТО ЧТО БЫЛО ПРИ ИНИЦИАЛИЗАЦИИ
...
% Stop the timer and clear the tcpObj after simulation
% stop(timerObj);
% disp("перед clear")
% clear timerObj
% clear tcpObj;
% disp("удаление таймеров")
